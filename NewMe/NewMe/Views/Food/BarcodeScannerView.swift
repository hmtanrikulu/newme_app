...
private struct NutritionGrid: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            nutriCell(value: Int(protein.rounded()), label: "protein (g)", color: .blue)
            Divider().frame(height: 36)
            nutriCell(value: Int(carbs.rounded()), label: "karb (g)", color: .orange)
            Divider().frame(height: 36)
            nutriCell(value: Int(fat.rounded()), label: "yağ (g)", color: .yellow)
        }
        .padding(.vertical, 12)
